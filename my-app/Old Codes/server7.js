...
app.put('/tasks/:task_id', (req, res) => {
    ...
    const updateSql = `
        ...
        WHERE task_id = ?;
    `;
    const updateData = [task_name, status_id, type_id, due_date, project_id, responsible_user_id || null, task_id];
...
app.get('/tasks/:task_id/subtasks', (req, res) => {
    const { task_id } = req.params;
    const sql = `
...
app.post('/tasks/:task_id/subtasks', (req, res) => {
    const { task_id } = req.params;
    const { subtask_name } = req.body;

    const insertSql = `
        INSERT INTO subtasks (task_id, subtask_name)
        VALUES (?, ?);
    `;
    db.query(insertSql, [task_id, subtask_name], (err, result) => {
        if (err) {
