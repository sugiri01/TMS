...
const Home = () => {
    ...
    const handleAddTask = () => {
        const taskData = {
            task_name: newTask || 'New task',  // If input is empty, default to 'New task'
            status_id: 1,   // Default status (New task)
            type_id: 1,     // Default type (Technical)
            due_date: null, // No due date set initially
        ...
        .then(response => {
            ...
            setNewTask('');  // Clear the input field after task is added
